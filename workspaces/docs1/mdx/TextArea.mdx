...
import { TextArea, useValidation, useAsyncValidation } from './../../usetheform/src';

# TextArea

Renders a _textarea_ element ([W3schools Textarea](https://www.w3schools.com/tags/tag_textarea.asp)) and accepts as props any html attribute listed at: [Html Textarea Attributes](https://www.w3schools.com/tags/tag_textarea.asp).
...
- Specifies the initial value of an _textarea_ element.

**`touched`**: boolean

- A field that has been touched/visited. Default value of _false_.

- If _true_, validation messages (sync and async) will be shown but only when the event onBlur of the field is triggered by a user action.
