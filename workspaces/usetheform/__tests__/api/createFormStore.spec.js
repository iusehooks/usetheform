...
describe("API => createFormStore", () => {
  ...
  it("should initiate the counter value from createFormStore", async () => {
    const [formStore] = createFormStore({ counter: "1" });

    const { getByTestId } = render(
      <div>
        <Form formStore={formStore} initialState={{ object: { age: "10" } }}>
          <Input type="text" name="counter" data-testid="counterInput" />
          <Collection object name="object">
            <Input type="text" name="age" data-testid="counterInputAge" />
          </Collection>
        </Form>
        <Counter selector={state => state.counter} />
      </div>
    ...
    const counterValue = getByTestId("counterInput");
    expect(counterValue.value).toBe("1");

    const counterValue2 = getByTestId("counterInputAge");
    expect(counterValue2.value).toBe("10");
  });
...
describe("API => withFieldSelector HOC", () => {
  it("should update the field value and trigger the listener", () => {
    const { subscribe, notify } = createListener();
    const useSelector = withFieldSelector(subscribe, {
      current: { state: { counter: "10" } },
      formStatus: { isFormMounted: false }
    });

    const jestFn = jest.fn(state => state.counter);
    const Counter = () => {
      useSelector(jestFn);
      return (
        <div>
    ...
    const counterBtn = getByTestId("counterBtn");
    act(() => {
...
describe("API => createFormStore Function", () => {
  it("should update the field value and trigger the listener if notify true", () => {
    ...
    act(() => {
      formStore.update({ state: { counter: "10" } }, false);
    });

    expect(jestFn).toHaveBeenCalledTimes(0);

    act(() => {
      formStore.mountForm(true);
      formStore.update({ state: { counter: "10" } }, true);
    });
