...
import { Item, recudeTotalPrice, recudeTotalQuantity } from "./helpers/Item";
...
A function invoked when the form has been resetted to its initial state.
...
<Playground>
  ...
  >
    ...
    <Input type="radio" name="sex" value="M" checked placeholder="M" />
    <Input type="radio" name="sex" value="F" placeholder="F" />
    <Input type="radio" name="sex" value="Other" placeholder="Other" />
...
```
<Playground>
  <Form reducers={[recudeTotalPrice, recudeTotalQuantity]}>
...
export const Item = ({ price, qty }) => {
  return (
    <Collection object>
      ...
      <Input type="number" name="qty" placeholder="Quantity" value={qty} />
    </Collection>
  );
}
